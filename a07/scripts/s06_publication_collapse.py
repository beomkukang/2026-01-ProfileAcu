"""
Step 6: Publication collapse sensitivity analysis.
Collapse records sharing the same PMID within a site using majority vote.
"""
import numpy as np
import pandas as pd
import os
...
def collapse_publications(df, disease_col, mechanism_col):
    """
    For each site+PMID combination, collapse to one record using majority vote.
    """
    collapsed_records = []
    for site in SITES:
        ...
        for pmid, group in site_df.groupby('PMID'):
            record = {
                'PMID': pmid,
                'Site': site,
                disease_col: majority_vote(group[disease_col]),
                mechanism_col: majority_vote(group[mechanism_col])
            }
...
def main():
    ...
    print("Collapsing publications within sites...")
    collapsed = collapse_publications(combined, 'disease_cat', 'mechanism_cat')

    # Save site-level collapse stats
    collapse_stats = []
    for site in SITES:
        n_orig = len(combined[combined['Site'] == site])
        n_collapsed = len(collapsed[collapsed['Site'] == site])
        n_pmids_collapsed = n_orig - n_collapsed
        collapse_stats.append({
            'site': site,
            'n_original': n_orig,
            'n_collapsed': n_collapsed,
            'n_pmids_collapsed': n_pmids_collapsed
        })

    stats_df = pd.DataFrame(collapse_stats)
    stats_path = os.path.join(OUTPUT_DIR, 'pub_collapse.csv')
    stats_df.to_csv(stats_path, index=False)
    print(f"Saved: {stats_path}")
    print(stats_df.to_string(index=False))

    # Compute Mantel r for collapsed data
    print("\nComputing Mantel test on collapsed data...")
    ...
    r_collapsed, p_collapsed = mantel_test(disease_rdm, mechanism_rdm, n_perms=10000)

    # Load or compute original
    original_r = None
    original_p = None
    ...
    mantel_results = pd.DataFrame([
        ...
        {'condition': 'pub_collapsed', 'mantel_r': r_collapsed, 'p_value': p_collapsed}
    ])

    mantel_path_out = os.path.join(OUTPUT_DIR, 'pub_collapse_mantel.csv')
    mantel_results.to_csv(mantel_path_out, index=False)
    print(f"\nSaved: {mantel_path_out}")
    print(mantel_results.to_string(index=False))
